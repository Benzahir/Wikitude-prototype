    ...
    public void OnScreenConversionComputed(bool success, Vector2 screenCoordinate, Vector3 pointCloudCoordinate) {
        if (success) {
                ...
                _augmentations.Add(newAugmentation);           
                    ...
                    {
                        for (int i = 1; i < _augmentations.ToArray().Length; i++)
                        {
                            drawLines();
                            guiText = (Vector3.Distance(_augmentations[i].transform.position, _augmentations[i-1].transform.position) * 100).ToString("F2");                            
                            MessageBox.text = guiText + "  "+ "cm";
                        }
                    }
        }
        else {
